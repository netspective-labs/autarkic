...
export function defaultTypedValues() {
  return {
    "text-list": (raw, _tag, _parents) => {
      const r = z.string().transform(splitCsv).parse(raw);
...
export class OmbBuilder {
  ...
  async buildFromHostElement(hostEl) {
    const src = hostEl.getAttribute?.("omb:src");
  ...
  #walkChildNodes(domParent, modelParent, isRoot, parents, host) {
    ...
    for (const node of childNodes) {
      ...
      if (node.nodeType === Node.ELEMENT_NODE) {
        ...
        // <omb:schema> is schema-only and ignored structurally
        const tagText = domTagText(el);
        if (String(tagText).toLowerCase() === "omb:schema") {
        ...
        // Parents chain is root..child (inclusive)
  ...
  #applyAttributesToNode(el, model) {
    for (const attr of Array.from(el.attributes)) {
      ...
      if (attr.name === "omb:type-as") {
        model[".tag"].ombTypeAs = attr.value;
        continue;
      }

      if (attr.name === "omb:src") continue;
